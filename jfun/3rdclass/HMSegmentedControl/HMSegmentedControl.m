...
- (void)drawRect:(CGRect)rect
{
    ...
    [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
        ...
        //add line
        [self drawLineStartAt:CGPointMake(self.segmentWidth * idx, self.height) stopAt:CGPointMake(self.segmentWidth * (idx+1), self.height)];
        if (idx<[self.sectionTitles count]-1)
                [self drawLineStartAt:CGPointMake(self.segmentWidth * (idx+1), 0) stopAt:CGPointMake(self.segmentWidth * (idx+1), self.height)];
        [self.layer addSublayer:self.selectedSegmentLayer];

    }];
}
- (void)drawLineStartAt:(CGPoint)a stopAt:(CGPoint)b{
